    ...
    def compute_temporal_embeddings(
        ...
        if self.use_memory:
            if self.memory_update_at_start:
                ...
                with torch.no_grad():
                    positives_t = torch.as_tensor(
                        positives, dtype=torch.long, device=self.memory.memory.device
                    )
                    persisted = self.memory.get_memory(positives_t)
                    if persisted.device != memory.device:
                        persisted = persisted.to(memory.device)
                    if positives_t.numel() > 0:
                        max_diff = (memory[positives_t] - persisted).abs().max().item()
                        if max_diff > 1e-4:
                            self.logger.warning(
                                f"Memory drift after update (max|Δ|={max_diff:.2e}); syncing snapshot."
                            )
                        memory[positives_t] = persisted
